...
export default function DepartmentsPage() {
  ...
  // Flatten all departments from all institutions
  const allDepartments = institutions.flatMap((inst) =>
  ...
  const [departments, setDepartments] = useState<(Department & { institutionName: string })[]>(allDepartments);
  ...
  const filteredDepartments = selectedInstitutionId === "all"
    ? departments
  ...
  const handleFormSubmit = (data: DepartmentInput) => {
    const inst = institutions.find((i) => i.id === data.institutionId);
    if (selectedDepartment) {
      setDepartments((prev) =>
        prev.map((dept) =>
          dept.id === selectedDepartment.id
            ? { ...dept, ...data, institutionName: inst?.name || "", updatedAt: new Date() }
            : dept
        )
      );
    } else {
      const newDepartment: Department & { institutionName: string } = {
        id: generateId("dept"),
        ...data,
        institutionName: inst?.name || "",
        teachers: [],
    ...
    <div className="space-y-6">
      ...
      {formDialogOpen && (
        <DepartmentFormDialog
          ...
          institutionId={selectedDepartment?.institutionId || (selectedInstitutionId !== "all" ? selectedInstitutionId : institutions[0]?.id || "")}
      ...
      <DeleteDialog
        ...
        description={`Are you sure you want to delete "${selectedDepartment?.name}"? This will also delete all teachers in this department. This action cannot be undone.`}
