...
export default function ProgramsPage() {
  ...
  // Flatten all programs from all institutions
  const allPrograms = institutions.flatMap((inst) =>
  ...
  const [programs, setPrograms] = useState<(Program & { institutionName: string })[]>(allPrograms);
  ...
  const filteredPrograms = selectedInstitutionId === "all"
    ? programs
      ...
      setPrograms((prev) =>
        prev.map((prog) =>
          prog.id === selectedProgram.id
            ? { ...prog, ...data, institutionName: inst?.name || "", updatedAt: new Date() }
            : prog
        )
      );
    } else {
      const newProgram: Program & { institutionName: string } = {
        id: generateId("prog"),
        ...data,
        institutionName: inst?.name || "",
        createdAt: new Date(),
    ...
    <div className="space-y-6">
      ...
      {formDialogOpen && (
        <ProgramFormDialog
          ...
          institutionId={selectedProgram?.institutionId || selectedInstitutionId !== "all" ? selectedInstitutionId : institutions[0]?.id || ""}
